...
export default function HomePage() {
  return (
    <div className="relative min-h-screen bg-gray-900 text-white overflow-hidden">
      ...
      {/* Header / Navbar */}
      <header className="relative z-10 bg-brand-900 bg-opacity-80 backdrop-blur-sm shadow-lg">
        <nav className="container mx-auto flex items-center justify-between p-4">
          ...
          <div className="flex space-x-6">
            <Link href="/" className="flex items-center pb-1 font-semibold text-brand-300 border-b-2 border-brand-300"><FiHome className="mr-1"/>Home</Link>
            <Link href="/about" className="flex items-center transition-colors hover:text-brand-200"><FiInfo className="mr-1"/>About us</Link>
      ...
      <main className="relative z-10 flex flex-col items-center justify-center min-h-[calc(100vh-64px)] p-4">
        <div className="animate-slide-up bg-white rounded-(--radius-card) p-8 md:p-12 text-center max-w-lg w-full shadow-(--shadow-elevated)">
          ...
          <Button href="/login" variant="primary" size="lg" className="rounded-(--radius-pill)">
            Try it <FiArrowRight className="ml-1 h-5 w-5" />
          </Button>
        </div>
      </main>
    </div>
